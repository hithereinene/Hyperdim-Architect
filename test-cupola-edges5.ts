import { generateHypercube, generateIcosahedron } from './services/mathUtils.ts';

const baseShape = generateHypercube(3);
const expShape = generateIcosahedron();
...
for (let tol of [0.05, 0.1, 0.2, 0.25, 0.3]) {
    let count = 0;
    for (let i = 0; i < v1.length; i++) {
        for (let j = 0; j < v2.length; j++) {
            let distSq = 0;
